...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [likes, liked, views, totalComments, rows] = await Promise.all([
      ...
      prisma.comment.findMany({ where: { postId: id, ...(cursorComment ? { OR: [{ createdAt: { lt: cursorComment.createdAt } }, { createdAt: cursorComment.createdAt, id: { lt: cursorComment.id } }] } : {}) }, orderBy: [{ createdAt: "desc" }, { id: "desc" }], take: 31,
