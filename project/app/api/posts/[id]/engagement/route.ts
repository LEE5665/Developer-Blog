  ...
  try {
    ...
    const [likes, liked, views, totalComments, rows] = await Promise.all([
      ...
      prisma.comment.findMany({ where: { postId: id }, ...(cursorComment ? { cursor: { id: cursorComment.id }, skip: 1 } : {}), orderBy: [{ createdAt: "asc" }, { id: "asc" }], take: 31,
        select: { id: true, parentId: true, content: true, nickname: true, anonymous: true, authorId: true, createdAt: true, updatedAt: true, author: { select: { name: true, nickname: true, image: true } } } }),
    ]);
    const comments = rows.slice(0, 30).map((comment) => ({ id: comment.id, parentId: comment.parentId, content: comment.content, nickname: comment.anonymous ? comment.nickname : comment.author?.nickname || comment.author?.name || comment.nickname, anonymous: comment.anonymous, image: comment.anonymous ? null : comment.author?.image || null, authorId: comment.anonymous ? null : comment.authorId, createdAt: comment.createdAt, updatedAt: comment.updatedAt, canEdit: comment.anonymous || (!!userId && comment.authorId === userId), canDelete: comment.anonymous || (!!userId && (comment.authorId === userId || post.authorId === userId)), canModerate: !!userId && post.authorId === userId }));
